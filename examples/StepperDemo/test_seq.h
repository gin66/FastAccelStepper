...
// Run 32000 steps with speed changes every 100ms in order to reproduce issue #24
bool test_seq_06(FastAccelStepper *stepper, struct test_seq_s *seq,
                 uint32_t time_ms);

// measures timing of several moveByAcceleration(). Should stop at position zero. (should be started from position 0).
bool test_seq_07(FastAccelStepper *stepper, struct test_seq_s *seq,
                 uint32_t time_ms);

// is an endless running test to check on esp32, if the generated pulses are successfully counted by a second pulse counter. The moves should be all executed in one second with alternating direction and varying speed/acceleration
bool test_seq_08(FastAccelStepper *stepper, struct test_seq_s *seq,
                 uint32_t time_ms);

// is an endless running test with starting a ramp with random speed/acceleration/direction, which after 1s is stopped with forceStopAndNewPosition(). It contains no internal test criteria, but looking at the log, the match of generated and sent pulses can be checked. And the needed steps for a forceStopAndNewPosition() can be derived out of this
bool test_seq_09(FastAccelStepper *stepper, struct test_seq_s *seq,
                 uint32_t time_ms);

// runs the stepper forward and every 200 ms changes speed with increasing positive speed deltas and then decreasing negative speed deltas.
bool test_seq_10(FastAccelStepper *stepper, struct test_seq_s *seq,
                 uint32_t time_ms);

// runs the stepper to position 1000000 and back to 0. This tests, if getCurrentPosition() is counting monotonously up or down respectively.
