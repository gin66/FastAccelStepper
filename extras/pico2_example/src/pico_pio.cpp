...
stepper_pio_program* stepper_make_program() {
  ...
  // ISR=position, X=period:dir:steps_to_do, Y=invalid,
  // OSR=period:dir:steps_to_do get steps_to_do into y (8 bits)
  ...
  // ISR=position, X=period:dir:steps_to_do, Y=steps_to_do,
  // OSR=period:dir:steps_to_do Forward jump, if steps_to_do is zero
  uint8_t forward_jump_1 = program.pc;
  add_step(pio_encode_jmp_not_y(0));
  // ISR=position, X=period:dir:steps_to_do, Y=[steps_to_do],
  // OSR=period:dir:steps_to_do Get the direction into Y for position update
  // below
  ...
  add_step(pio_encode_mov(pio_osr, pio_x));
  // ISR=position, X=period:dir:steps_to_do-1, Y=dir,
  // OSR=period:dir:steps_to_do-1 set step output to 1 aka HIGH
  ...
  // restore x to osr
  add_step(pio_encode_mov(pio_x, pio_osr));
  // ISR=position+1, X=period:dir:steps_to_do, Y=[dir],
  // OSR=period:dir:steps_to_do Jump to the start of the loop
  ...
  // ISR=position, X=period:dir:steps_to_do, Y=undefined,
  // OSR=period:dir:steps_to_do isolate period into osr
  ...
  add_step(pio_encode_mov(pio_osr, pio_x));
  // ISR=position, X=[period:dir:steps_to_do], Y=period,
  // OSR=period:dir:steps_to_do move position into x
  add_step(pio_encode_mov(pio_x, pio_isr));
