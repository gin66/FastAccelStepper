...
void setup() {
  ...
  bool rc = pio_claim_free_sm_and_add_program_for_gpio_range(&pio_program, &pio, &sm, &offset, PIO_STEP_GPIO, 1, true);

  pio_sm_config c = pio_get_default_sm_config();
  // Map the state machine's OUT pin group to one pin, namely the `pin`
  // parameter to this function.
  sm_config_set_jmp_pin(&c, PIO_STEP_GPIO);    // Step pin read back for double period
...
void loop() {
  ...
  // this pause is toggling for couple of repeats beween all low and one high/low part.
  steps = 0;
  period = stepper_calc_period(forward, steps, 64000); // 4ms
  ...
  if ((cnt % 10) == 0) {
    ...
    // we expect a new value in the fifo within less than 1 us. So the Serial.print is sufficient delay
