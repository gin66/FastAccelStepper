...
void perform_test() {
  ...
  x = upm_sqrt(x1);
  back = upm_to_u32(x);
  xprintf("upm_sqrt(%x)=0x%x (%d)  = 362 ?\n", x1, x, back);
  test(back == 362, "upm_sqrt");

  x1 = upm_from((uint32_t)0x10000);
  x = upm_sqrt(x1);
  back = upm_to_u32(x);
  xprintf("upm_sqrt(%x)=0x%x (%x)\n", x1, x, back);
  ...
  back = upm_to_u32(x);
  xprintf("upm_shl(upm_sqrt(upm_shr(%x,2)),1)=0x%x (%x)\n", x1, x, back);
  test(back == 2, "upm_sqrt");
  x = upm_shl(upm_sqrt(upm_shr(x1, 4)), 2);
  back = upm_to_u32(x);
  xprintf("upm_shl(upm_sqrt(upm_shr(%x,4)),2)=0x%x (%x)\n", x1, x, back);
  x = upm_shl(upm_sqrt(upm_shr(x1, 42)), 21);
  back = upm_to_u32(x);
  xprintf("upm_shl(upm_sqrt(upm_shr(%x,42)),21)=0x%x (%x)\n", x1, x, back);
  test(back == 2, "upm_sqrt");
