...
void IRAM_ATTR i2s_clear_block(uint8_t* buf, struct i2s_fill_state* state,
  ...
  if (pulse_width_bits == 0)
    pulse_width_bits = I2S_DEFAULT_PULSE_WIDTH_TICKS / 2;

  uint8_t pulses = state->pulse_count[blk];
  uint8_t bits_to_clear = pulse_width_bits;
  for (uint8_t i = 0; i < pulses; i++) {
    uint16_t bit_pos = state->pulse_positions[blk][i];
    for (uint8_t b = 0; b < bits_to_clear; b++) {
      uint16_t byte_idx = (bit_pos + b) >> 3;
      if (byte_idx >= I2S_BYTES_PER_BLOCK) {
        break;
      }
      uint8_t bit_mask = 0x80 >> ((bit_pos + b) & 7);
