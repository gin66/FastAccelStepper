  ...
  if (rp == q->next_write_idx) {
	...
	if (steps == 0) {
		...
		*data = rmt_entry;
		...
		// special treatment for this case, because an rmt entry can only cover up to 0xfffe ticks
		// every step must be minimum split into two rmt entries, so at max PART/2 steps can be done.
		...
		*last_step_data = rmt_entry;
		...
		// This could lead to single step at high speed in next part .... so need to rework
		// The previous part tries to address this
		steps -= PART_SIZE;
	}
	if (steps == 0) {
	  ...
	  // The dir pin toggle at this place is problematic, but if the last command contains only one step, it could work
...
void StepperQueue::init_rmt(uint8_t channel_num, uint8_t step_pin) {
  ...
  rmt_rx_stop(channel);
  if (channel_num == 0) {
      rmt_isr_register(tx_intr_handler, NULL, ESP_INTR_FLAG_SHARED | ESP_INTR_FLAG_IRAM, NULL);
